...
<div class="form-div">
    <form action="refund_stats.php" name="searchForm">
        ...
        <select id="sel_shop" name="sel_shop" onchange="change_shop()">
            <option value="0" <?php if ($this->_var['sel_shop'] == '0'): ?> selected<?php endif; ?>>整站</option>
            <option value="1" <?php if ($this->_var['sel_shop'] == '1'): ?> selected<?php endif; ?>>平台自营</option>
            <option value="2" <?php if ($this->_var['sel_shop'] == '2'): ?> selected<?php endif; ?>>第三方店铺</option>
        </select>
        <label id="notice"></label>
        <select id="supplier_id" class="chzn-select" name="supplier_id" style="height:100;">
            <option value="0">全部</option>
            <?php echo $this->html_options(array('options'=>$this->_var['suppliers_list_name'],'selected'=>$this->_var['supplier_id'])); ?>
        </select>
        <br />
...
?>
    ...
    function batch_export()
    {
        ...
        return location.href='refund_stats.php?act=export&stats_type=' +
